...
main(int argc, char *argv[])
{
	...
	if (ei_connect_xinit(&EI_LUA_STATE.ec, lua_host, lua_node, fullnodeid, addr, cookie, 0) < 0) {
		print("EI initialisation failed");
		exit(4);
	}
	...
	if ((EI_LUA_STATE.fd = ei_connect(&EI_LUA_STATE.ec, EI_LUA_STATE.erlang_node)) < 0) {
		print("Cannot connect to parent node '%s'",
				EI_LUA_STATE.erlang_node);
		exit(5);
	}
	if (! start_lua())
		exit(6);
...
reconnect()
{
	...
	if ((EI_LUA_STATE.fd = ei_connect(&EI_LUA_STATE.ec, EI_LUA_STATE.erlang_node)) < 0) {
		print("FATAL: Cannot reconnect to parent node '%s'",
				EI_LUA_STATE.erlang_node);
		exit(7);
	}
...
main_message_loop()
{
	...
	while (running) {
		...
		case ERL_ERROR:
		default:
			print("DEBUG: Lua Erlang Node error in receive");
			reconnect();
			break;
		case ERL_TICK:
...
lerl_rpc(lua_State *L)
{
	...
	if (ei_rpc(&EI_LUA_STATE.ec, EI_LUA_STATE.fd, (char *) mod, (char *) fun,
		...
		print("Warning: erl_rpc(%s, %s, ...) call error.",
			mod, fun);
		lua_pushfstring(L, "erl_rpc(%s, %s, ...) call error.",
			mod, fun);
		lua_error(L);
	}
	x_rpc_out->index = 0;
